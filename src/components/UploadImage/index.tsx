...
import { PlusOutlined, UploadOutlined, WarningFilled, CloseCircleFilled } from '@ant-design/icons'
...
const getItem = (item: any, index: number, props: any) => {
  const { handleRemove, handlePreview, onChangeError, percent = {}, fileList, isCanRemove } = props
    ...
    return (
      <div className="upload-photo-image">
        <div className="upload-photo-error">
          ...
          <div className="upload-photo-upload--agin" onClick={() => {
    ...
    return (
      <div className="upload-photo-uploading--container">
        <div className="upload-photo-uploading">
          <Progress percent={percent[item.uid] || 40} showInfo={false} strokeWidth={4} />
        </div>
      </div>
    )
  }
  ...
  const defaultBolck = (item: any) => {
    return (
      <div className="upload-photo-image">
        {
          item.url ?
            <img alt="图片" src={item.url} onClick={() => {
              return handlePreview && handlePreview(item)
            }} />
            : <div className="upload-no-image">
              {setting.uploadImage.noData}
            </div>
        }
        {isCanRemove && <CloseCircleFilled onClick={() => {
          return handleRemove && handleRemove(item)
        }} className="upload-photo-del" />}
      </div>
    )
...
const UploadImage = forwardRef((props: IUploadImageProps, ref) => {
  console.log(props)
  const { headers, data, action, initialValues, minSizeWidth, maxSizeWidth, minSizeHeight, maxSizeHeight } = props
  const { isDragSort, lineSize, explain, multiple, maxCount, disabled, maxSize, onChange, isShowImg, placeholder, direction } = props
  ...
  const uploadButton = (
    <div>
      <PlusOutlined />
      {isDragSort && <div className="ant-upload-text">{placeholder || setting.uploadImage.canDragDrop}</div>}
    </div>
  )
  ...
  const handleBeforeUpload = (file: any, fileLists: any) => {
    ...
    if (maxCount < fileLists.length + fileList.length) {
      message.error(`${setting.uploadImage.maxCountLimit}${maxCount - fileList.length}${setting.uploadImage.zhang}`)
  ...
  const isSizeImage = (nw: number, nh: number) => {
    if (!!minSizeWidth && minSizeWidth > nw) {
      Modal.warning({ title: `${setting.uploadImage.widthNoLessThan}${minSizeWidth}${setting.uploadImage.pixel}（px）` })
      return false
    }
    if (!!maxSizeWidth && maxSizeWidth < nw) {
      Modal.warning({ title: `${setting.uploadImage.widthNoMoreThan}${maxSizeWidth}${setting.uploadImage.pixel}（px）` })
      return false
    }
    if (!!minSizeHeight && minSizeHeight > nh) {
      Modal.warning({ title: `${setting.uploadImage.heightNoLessThan}${minSizeHeight}${setting.uploadImage.pixel}（px）` })
      return false
    }
    if (!!maxSizeHeight && maxSizeHeight < nh) {
      Modal.warning({ title: `${setting.uploadImage.heightNoMoreThan}${maxSizeHeight}${setting.uploadImage.pixel}（px）` })
      return false
    }
    let flag = true
  ...
  return (
    <div ref={ref} className="dp-img-upload" style={{ maxWidth: `96*${lineSize}px` }}>
